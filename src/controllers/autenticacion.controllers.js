...
export const validarUsuario = async (req, res) => {
    try {
        if (!req.body || !req.body.identificacion || !req.body.contraseña) {
            return res.status(200).json({ status: 400, message: 'Falta la identificación o contraseña en la solicitud' });
        }
        ...
        console.log([identificacion, contraseña])
        if (rows.length > 0) {
            ...
            return res.status(200).json( { status: 401, message: 'Usuario no encontrado...' });
        }
    } catch (e) {
        console.error('Error en el sistema:', e);
        return res.status(200).json({ status: 500, message: 'Error en el sistema' });
    }
};
