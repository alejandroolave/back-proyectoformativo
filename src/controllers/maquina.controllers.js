...
export const registrarMaquina = async (req, res) => {
    try {
        console.log('MQUIN: ', req.files);

        let error1 = validationResult(req);
        if (!error1.isEmpty()) {
            return res.status(200).json(error1);
        }
        const { nombre, marca, placa, modelo, cantidad, serial, descripcion, id_usuario, id_area, id_ambiente } = req.body;
        let imagen = "";
        let manual = "";
        if (!req.files || Object.keys(req.files).length === 0) {
            return res.status(200).json({
                errors: [{ type: "field", value: "", msg: "Archivo requerido", path: "file", location: "body" }],
                status: false
            });
        ...
        const sql = `INSERT INTO maquina(nombre, marca, placa, modelo, cantidad, manual, serial, imagen, descripcion, id_usuario, id_area, id_ambiente)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;

        const values = [nombre, marca, placa, modelo, cantidad, manual, serial, imagen, descripcion, id_usuario, id_area, id_ambiente];
